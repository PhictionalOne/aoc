...
strategy :: String -> (Char, Char)
strategy (o:' ':m) = (o,head m)
strategy x = error x


partOne :: String -> Int
partOne = sum . map score
        . map strategy . init . splitOn "\n"
  where score :: (Char, Char) -> Int
        score ('A', 'X') = 1 + 3
        score ('A', 'Y') = 2 + 6
        score ('A', 'Z') = 3 + 0
        score ('B', 'X') = 1 + 0
...
- In the third round, you will defeat your opponent's Scissors with Rock for a
  score of 1 + 6 = 7.  Now that you're correctly decrypting the ultra top secret
